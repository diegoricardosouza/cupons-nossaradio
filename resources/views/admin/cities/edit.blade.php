<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <ul class="flex items-center mt-5">
            ...
            <li>
                <a href="{{ route('cities.index') }}" class="text-sm font-semibold text-neutral-500 transition-all hover:text-violet-900">Cidades</a>
            </li>
            <li class="mx-1 text-neutral-400">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
            ...
            <li class="text-sm text-violet-900 font-semibold">
                Editar Cidade <span class="font-bold">{{ $city->name }}</span>
            </li>
        </ul>
    </div>
                ...
                <form action="{{ route('cities.update', $city->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div>
                        <label class="block">
                            ...
                            <input type="text" name="name" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" value="{{ $city->name ?? old('name') }}" />
                        </label>
