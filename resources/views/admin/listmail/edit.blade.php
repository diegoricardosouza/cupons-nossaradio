    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <ul class="flex items-center mt-5">
            ...
            <li class="text-sm text-violet-900 font-semibold">
                Editar Email <span class="font-bold">{{ $mail->email }}</span>
            </li>
        </ul>
    </div>
    ...
    <div class="py-5">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg px-5 py-5">
                <form action="{{ route('list.update', $mail->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                        ...
                        <div class="flex-auto">
                            <div>
                                <label class="block">
                                    ...
                                    <input type="email" name="email" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" value="{{ $mail->email ?? old('email') }}" />
                                </label>
